...
class Tensor
{
  ...
  private:
	...
	{
		if (std::is_void_v<T>) {
			return; // you can always cast to void
		}
  ...
  public:
     ...
     */
	Tensor()
	    : hostsData_(false), data_(nullptr), precision_(FLOAT), ndim_(0)
	{
	}
	...
	Tensor(void* data, Precision precision, const std::vector<int32_t>& sizes, const std::vector<int32_t>& strides)
	    : hostsData_(false), data_(data), precision_(precision), ndim_(static_cast<int>(sizes.size())), sizes_(sizes), strides_(strides)
	{
		assert(!sizes.empty());
	...
	{
		return Accessor(static_cast<typename Accessor::PtrType>(
		                                    dataPtr<typename Accessor::Type>()),
		                sizes().data(), strides().data());
	}
};
